...
#
# Copyright 2024-2026 by it's authors.
...
def on_object_created(instance, event):
    ...
    """
    # get the contained samples
    samples = instance.getContainedSamples()
    # maybe a single-results report?
    parent = api.get_parent(instance)
    if IAnalysisRequest.providedBy(parent):
        if parent not in samples:
            samples.append(parent)
    # notify Tamanu about the diagnostic report for each sample
